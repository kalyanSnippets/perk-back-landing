...
const MerchantCustomers = () => {
  ...
  const filtered = search.trim()
    ? customers.filter(c =>
        ...
        c.loyalty_card_number.includes(search)
      )
    : customers;
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <p className="text-muted-foreground text-sm animate-pulse">Loading customers...</p>
      </div>
    );
  }
                  ...
                  <div key={c.id} className="flex items-center justify-between p-4 hover:bg-muted/30 transition-colors">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[11px] text-muted-foreground flex items-center gap-1 mt-0.5">
                        <CreditCard size={10} /> {c.loyalty_card_number}
                      </p>
                    </div>
                  </div>
                ))
